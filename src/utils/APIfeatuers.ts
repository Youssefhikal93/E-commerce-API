...
export class APIFeatures {
    ...
    private filterBy?: 'price' | 'quantity' | 'longDescription' | 'shortDescription' | 'name' | 'categoryId' | 'shopId'
    ...
    private validate() {
        ...
        if (!validSortByFields.includes(this.sortBy!)) {
            throw new BadRequestException(
                `Invalid sortBy value: ${this.sortBy}. Valid options are ${validSortByFields.join(', ')}`
            )
        }
        ...
        if (this.filterCondition && this.filterBy) {
            ...
            if (validStringFields.includes(this.filterBy) && !stringFilterConditions.includes(this.filterCondition)) {
                throw new BadRequestException(
                    `Invalid filterCondition for string field: ${this.filterCondition}. Valid options are '${stringFilterConditions.join(', ')}'`
                )
            }

            if (validNumericFields.includes(this.filterBy) && !numericFilterConditions.includes(this.filterCondition)) {
                throw new BadRequestException(
                    `Invalid filterCondition for numeric field: ${this.filterCondition}. Valid options are '${numericFilterConditions.join(', ')}'`
                )
            }

            const validFilterValueFields = [
                'price',
                'quantity',
                'longDescription',
                'shortDescription',
            ...
            if (this.filterBy && !validFilterValueFields.includes(this.filterBy)) {
                throw new BadRequestException(
                    `Invalid filterValue value: ${this.filterBy}. Valid options are '${validFilterValueFields.join(', ')}'`
                )
            }
        }
    }
    ...
    public getWhereClause() {
        ...
        if (this.filterBy && this.filterCondition && this.filterValue) {
            if (this.filterBy === 'price' || this.filterBy === 'quantity' || this.filterBy === 'categoryId' || this.filterBy === 'shopId') {
